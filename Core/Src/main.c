...
int main(void)
{
  ...
  printfln("printfln:系统初始化完成");
  CoolingCreate(&huart2);
  printfln("CoolingCreate初始化完成");
  ...
  HAL_Delay(1);
  printfln("htim6启动，开始液冷控制器周期调度");
  BAT_DATA_Pack.BatID = 1;
  printfln("配置BatID为%d",BAT_DATA_Pack.BatID);
  while (1)
  {
    /* USER CODE END WHILE */
